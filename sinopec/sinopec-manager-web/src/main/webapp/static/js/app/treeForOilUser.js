...
//树的配置
var setting_ForOilUser = {
		treeId: "treeForOilUser",//用来区别不同的树
		view: {
         ...
         callback: {
        	 ...
        	 beforeClick: function(treeId, treeNode) {
                 var zTree = $.fn.zTree.getZTreeObj("treeForOilUser");
             ...
             onClick:function(event, treeId, treeNode) {
            	 ...
            	 $.ajax({
                     ...
                     success : function(TableHead) {
                    	...
                    	str = "<tr>"+str+"<td><pre>操作</pre></td>"+"<td><pre>操作</pre></td>"+"</tr>";
...
$.ajax({
          ...
          success : function(TypeTree) {
        	  ...
        	  $(document).ready(function(){
        		   ...
        		   * 
        		   */
        		  t = $.fn.zTree.init(t, setting_ForOilUser, zNodes);
        	  });
          },
          error : function() {
        	  alert("获取角色失败!","提示信息");
...
//筛选按钮的响应事件
//测试用数据:32650329--中国石化石家庄分公司高新片区黄河大道加油站###1;32650314--中国石化石家庄分公司长安片区建设大街加油站###67
function choose(){
	if(dcid == null){
		alert("请选择要查看的基础数据");
		return;
			...
			success : function(values) {
				...
				if(values.length == 2){
					...
					for(var obj in objs){
						...
						strs = "<tr class='"+objs[obj].instanceId+"'>"+"</td>"+str
...
function confirmdata(insid){
	...
	$("."+insid).find("td").each(function(j){
		if(j<TableHeads.length){
			instance_value_new = $("#instance_value_newinput_"+TableHeads[j].id).val();
			$("."+insid+" td:nth-child("+(j+1)+") pre").empty();
			$("."+insid+" td:nth-child("+(j+1)+") pre").html(instance_value_new);
		}else if(j == TableHeads.length){
	...
	$("."+insid).find("td").each(function(j){
		...
		if(instance_value =="暂无数据" || j>TableHeads.length-1){
			//不操作
		}else{
			var data = {};
...
function editdata(insid){
	...
	$("."+insid).find("td").each(function(j){
		if(j<TableHeads.length){
			...
			.html("<input id='instance_value_newinput_"+TableHeads[j].id+"' type='text' value='"+instance_value_new+"'/>");
		}else if(j == TableHeads.length){
			$("."+insid+" td:nth-child("+(j+1)+") pre").toggle();
